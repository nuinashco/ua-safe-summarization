...
export GPU_TYPE="${GPU_TYPE:-nvidia-tesla-t4}"
export GPU_COUNT="${GPU_COUNT:-1}"
export PROVISIONING_MODEL="${PROVISIONING_MODEL:-STANDARD}"
export CPU_MILLI="${CPU_MILLI:-8000}"
export MEMORY_MIB="${MEMORY_MIB:-30000}"
export BOOT_DISK_SIZE_GB="${BOOT_DISK_SIZE_GB:-250}"
export SHM_SIZE="${SHM_SIZE:-1g}"
export RESERVATION="${RESERVATION:-NO_RESERVATION}"
export LOGS_DESTINATION="${LOGS_DESTINATION:-CLOUD_LOGGING}"

# Optional: keep Batch inside regions/zones that offer T4.
export ALLOWED_LOCATION="${ALLOWED_LOCATION:-regions/${LOCATION}}"
